...
function getOffset(pList, k) {
  ...
  temp_path = Clipper.Clipper.SimplifyPolygon(
    temp_path,
    Clipper.PolyFillType.pftPositive
  );
...
function thicken(pList, k, scaleFactor, openButt, miterLimit) {
  ...
  const endType = openButt
    ? Clipper.EndType.etOpenButt
    : Clipper.EndType.etOpenSquare;
...
function getDifferenceMult(polygons, cutters) {
  ...
  polygons.forEach((polygon) =>
    differences.push(...getDifference(polygon, cutters))
  );
